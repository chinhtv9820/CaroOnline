...
public interface IGameService
{
    ...
    Task<int> EnsurePlayerAsync(int gameId, int userId, CancellationToken ct = default);
    Task<Message> SendMessageAsync(int gameId, int senderId, string content, CancellationToken ct = default);
    Task<List<Message>> GetMessagesAsync(int gameId, CancellationToken ct = default);
}
